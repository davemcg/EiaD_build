...
background_go_words <- function(go_enrichment, ontology){
  all_keywords <- all_vs_all_go %>% filter(ONTOLOGY=='BP') %>% dplyr::select(ID, Description) %>% unique() %>% pull(Description)
...
cloud_maker <- function(word_vector, exclusion_words, max.words, all_go, title){
  ...
  png(filename = paste0(output_folder, '/',title), width = 400, height = 400, res = 300, pointsize = 3)
...
all_vs_all_go %>%
  group_by(Set, Test) %>% 
  filter(as.numeric(p.adjust) < 0.01, ONTOLOGY=='BP') %>% 
  mutate(Title = paste(Set, Test, sep='__'), Title=paste0(Title, '.png')) %>% 
  do(cloud_maker(.$Description,c('involved','process','regulation','negative','positive'),75,all_BP_go,.$Title))
