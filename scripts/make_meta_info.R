...
SRP105756 <- SRP105756 %>% mutate(study_title = 'A gene expression study of the developing human eye',
                                  ...
                                  study_accession = 'SRP105756') %>% rename(sample_accession = SRA_Sample, run_accession = Run) %>% select(sample_accession, run_accession, sample_attribute, study_title, study_accession, study_abstract)
...
core_tight <- left_join(core_tight, bind_rows(SRP159246,SRP119766, amd, SRP105756), by = "sample_accession") %>% 
  mutate(study_title = case_when(is.na(study_title.x) ~ study_title.y, 
         ...
         run_accession = case_when(is.na(run_accession.x) ~ run_accession.y,
