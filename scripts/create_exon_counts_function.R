    ...
    for (i in projects_vector) {
      ...
      while( is.null(success) && attempt <= 3 ) {
        try({
          ...
          attempt <- attempt + 1
          if (GTEX){
            rse_gene = create_rse(subset(gtex_data, project == i), annotation = "gencode_v29", type = type)
          } else {
            rse_gene = create_rse_manual(i, annotation = "gencode_v29", type = type)
          }
