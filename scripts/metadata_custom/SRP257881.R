...
srr <- read_csv('data/SRP257881.sra.txt')
...
for (i in srr %>% pull(Run) ){
  ...
  SRP257881_list[[i]] <- out %>% data.frame()
  Sys.sleep(1) 
}

SRP257881_meta <- SRP257881_list %>% bind_rows() %>% 
  ...
  mutate(sample_accession = Pool.Member.IDENTIFIERS.PRIMARY_ID, 
         ...
         Tissue = 'RPE', 
         Source = 'Native',
         Age = case_when(Age == 'infant' ~ 'Infant',
                         TRUE ~ 'Adult'),
         ...
         region = NA) %>% select(sample_accession:region, run_accession)
