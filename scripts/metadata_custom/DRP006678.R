...
srr <- read_csv('data/DRP006678.sra.txt')
...
for (i in srr %>% pull(Run) ){
  ...
  DRP006678_list[[i]] <- out %>% data.frame()
  Sys.sleep(1) 
}

DRP006678_meta <- DRP006678_list %>% bind_rows() %>% 
...
DRP006678_meta <- DRP006678_meta[,colnames(DRP006678_meta)[!grepl('\\.', colnames(DRP006678_meta))]]
