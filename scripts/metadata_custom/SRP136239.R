...
srr <- read_csv('data/SRP136239.sra.txt')


SRP136239_list <- list()
for (i in srr %>% filter(Experiment %in% c('SRX9565492','SRX9565493','SRX9565494','SRX9565495')) %>% pull(Run) ){
  print(i)
  ...
  SRP136239_list[[i]] <- out %>% data.frame()
  Sys.sleep(1) 
}

SRP136239_meta <- SRP136239_list %>% bind_rows() %>% 
  mutate(BioSample = Pool.Member.IDENTIFIERS.EXTERNAL_ID) %>% 
  left_join(srr %>% mutate(run_accession = Run)) %>% 
...
SRP136239_meta <- SRP136239_meta[,colnames(SRP136239_meta)[!grepl('\\.', colnames(SRP136239_meta))]]
