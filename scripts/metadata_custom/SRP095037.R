...
srr <- read_csv('data/SRP095037.sra.txt')


SRP095037_list <- list()
for (i in srr %>% filter(LibrarySelection == "cDNA", Age == "differentiation day 35") %>% pull(Run) ){
  print(i)
  ...
  SRP095037_list[[i]] <- out %>% data.frame()
  Sys.sleep(1) 
}

SRP095037_meta <- SRP095037_list %>% bind_rows() %>% mutate(sample_accession = Pool.Member.IDENTIFIERS.PRIMARY_ID, 
