...
srr <- read_csv('data/SRP011895.sra.txt')


SRP011895_list <- list()
for (i in srr %>% filter(origen_of_cultured_cells == "RPE isolated from 16 weeks gestation human fetus") %>% pull(Run) ){
  print(i)
  ...
  SRP011895_list[[i]] <- out %>% data.frame()
  Sys.sleep(1) 
}

SRP011895_meta <- SRP011895_list %>% bind_rows() %>% mutate(sample_accession = Pool.Member.IDENTIFIERS.PRIMARY_ID, 
