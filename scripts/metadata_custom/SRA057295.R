...
# a few non-wounded included
library(reutils)
library(xml2)
library(tidyverse)

srr <- read_csv('data/SRP057295.sra.txt')


SRP057295_list <- list()
for (i in srr %>% filter(Treatment == 'None', substrate == 'Microporous support') %>% pull(Run) ){
  print(i)
  ...
  SRP057295_list[[i]] <- out %>% data.frame()
  Sys.sleep(1) 
}

SRP057295_meta <- SRP057295_list %>% bind_rows() %>% mutate(sample_accession = Pool.Member.IDENTIFIERS.PRIMARY_ID, 
