...
srr <- read_csv('data/SRP318902.sra.txt')


SRP318902_list <- list()
for (i in srr %>% filter(Run %in% c('SRR14466172','SRR14466173')) %>% pull(Run) ){
  print(i)
  ...
  SRP318902_list[[i]] <- out %>% data.frame()
  Sys.sleep(1) 
}

SRP318902_meta <- SRP318902_list %>% bind_rows() %>% 
...
SRP318902_meta <- SRP318902_meta[,colnames(SRP318902_meta)[!grepl('\\.', colnames(SRP318902_meta))]]
