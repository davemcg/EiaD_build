...
srr <- read_csv('data/SRP090027.sra.txt')


SRP090027_list <- list()
for (i in srr %>% filter(grepl("scramble", `Library Name`)) %>% pull(Run) ){
  print(i)
...
SRP090027_meta <- SRP090027_list %>% bind_rows() %>% 
  ...
  mutate(sample_accession = Pool.Member.IDENTIFIERS.PRIMARY_ID, 
         ...
         data_location = 'recount3',
         #Library_Notes = EXPERIMENT.DESIGN.LIBRARY_DESCRIPTOR.LIBRARY_CONSTRUCTION_PROTOCOL,
         region = NA) 

SRP090027_meta <- SRP090027_meta[,colnames(SRP090027_meta)[!grepl('\\.', colnames(SRP090027_meta))]]
